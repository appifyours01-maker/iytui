...
class CartManager extends ChangeNotifier {
  ...
  double get totalDiscount => _items.fold(0.0, (sum, item) {
  ...
  Future<void> _loadCart() async {
    try {
      ...
      final raw = prefs.getString('cart_items_v1');
...
// API Configuration
class ApiConfig {
  static String get baseUrl => dotenv.env['API_BASE'] ?? 'http://192.168.0.9:5000';
  static const String adminObjectId = '695fa5ceda950cba5d856267';
...
class SessionManager {
  ...
  static Future<void> bindAuth({
    ...
    await MobileSessionManager().loadProfileImage();
  }
  ...
  Future<void> _fetchAppNameAndNavigate() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${dotenv.env['API_BASE'] ?? ApiConfig.baseUrl}/api/admin/splash?adminId=${adminId}&appId=${ApiConfig.appId}'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          if (token != null && token.isNotEmpty && userId != null && userId.isNotEmpty) {
            await SessionManager.bindAuth(
              userId: userId, 
              token: token,
              profileImg: user?['profileImage']?.toString(),
            );
          }
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        // Show stock label for admin users only
                        if (!isSoldOut)
                          FutureBuilder<bool>(
